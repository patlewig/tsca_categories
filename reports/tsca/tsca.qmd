...
Using a deep learning graph attention mechanism network developed in Adams et al, *submitted*, predictions were made for the TSCA landscape to profile the likely NAM profile with respect to a multi-component assay platform within ToxCast. Selected assays focused on one assay platform, Attagene (ATG), which provides a multiplexed read-out of transcription factor activity for a number of receptors transformed into HepG2 cells [@medvedev_2018]. A multi-task graph based approach as implemented within the DeepChem python library was used to develop the model that predicted the probability of a substance resulting in a positive hitcall.
...
@fig-vega1 (a - d) represent terminal categories ('Pyridines and derivatives', 1.0), ('Benzene and substituted derivatives', 9.0), ('Benzene and substituted derivatives', 9.0) and ('Lactones', nan) respectively.

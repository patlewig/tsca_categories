...
### Predicted Structural Alert Profile using Derek Nexus
...
### Predicted Structural Alert Profile using the OECD Toolbox

Selected profilers available within the OECD Toolbox v4.6 were used to screen the entire set of TSCA landscape chemicals (as parents) for potential alerts. The profilers selected were acute oral toxicity, the carcinogenicity alerts by ISS, the DART scheme from Procter & Gamble, the various OECD and OASIS alerts for DNA and protein binding, the Estrogen and retinoic acid receptor binding, Eye and Skin irritation/corrosion inclusion rules. For each profiler, the corresponding Toolbox output provides the specific structural alert(s) flagged for a given chemical in each case. This was transformed into a fingerprint representation such that a new matrix was constructed where all endpoint-alerts combinations formed columns and each cell was populated with a 1 or 0 to indicate presence or absence of that endpoint-alert combination. This format could then be evaluated through the perspective of the terminal categories 1) to visually evaluate the consistency in the alert profiles across a category and 2) to identify what endpoints were overrepresented in specific categories.
...
## Profiling structural alerts from the OECD Toolbox

OECD Toolbox profiling outcomes were generated for 12,085 substances from the TSCA landscape and summarised in a bit vector format so that each substance was associated with a fingerprint made up of all possible endpoint-alert combinations present. There were 2796 unique end-point-toxicophore combinations encompassing 26 different profilers which captured 11 main endpoints (nuclear receptor binding (retinoic acid and estrogen receptor binding), acute oral toxicity, respiratory uncouplers, systemic toxicity, genotoxicity, skin and respiratory sensitisation, carcinogenicity, developmental & reproductive toxicity, skin and eye irritation). The 318 substances (of which only 276 were associated with outcomes from the Toolbox) flagged 618 of these endpoint-alert combinations encompassing all 11 endpoints.

Pairwise distance matrices of the Derek fingerprints were computed to explore the extent to which there was consistency in alert profile across each terminal category. Four terminal categories are illustrated in @fig-toolbox to show the extent to which alert profiles varied.

```{python}
import pickle

dists = pickle.load(open(interim_dir+ "dists_toolbx_250624.pkl", "rb" ) )
```
...
@fig-toolbox (a - d) represent terminal categories ('Pyridines and derivatives', 1.0), ('Benzene and substituted derivatives', 9.0), ('Benzene and substituted derivatives', 9.0) and ('Lactones', nan) respectively.

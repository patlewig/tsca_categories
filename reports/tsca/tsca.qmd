...
Under the Toxic Substances Control Act (TSCA), as amended by the Frank R. Lautenberg Chemical Safety for the 21st Century Act [@usepa_2016], the US Environmental Protection Agency (EPA) evaluates potential risks from new and existing chemicals and acts to address any unreasonable risks that chemicals may have on human health and the environment. The non-confidential TSCA Inventory contains 86,741 chemicals, of which 42,293 are active (at the time of writing February 2024) in commerce which renders the use of traditional approaches too resource and time intensive to generate relevant data to facilitate assessment. In 2022, EPA launched a new effort under TSCA to modernise the process and bring innovative science to the review of new chemicals before they are permitted to enter the marketplace. This included a multi-year collaborative research programme (known as the New Chemical Collaborative Research Program (NCCRP)) to refine existing approaches as well as develop and implement New Approach Methods (NAMs) to ensure the best available science could be applied in new chemical evaluations. Several key areas were proposed to: 1) refine read-across approaches; 2) digitise and consolidate existing information on chemicals and combine them with publicly available sources to broaden coverage and accessibility; 3) update Quantitative Structure Activity Relationship (QSAR) models for physicochemical and (eco)toxicological properties; 4) explore ways of integrating and applying NAMs in new chemical assessments thereby reducing the reliance on animal testing and 5) develop decision support tools to integrate various data streams to facilitate new chemical risk assessment [@usepa_2022]. In terms of actualising how *in vitro* NAMs would be integrated and applied, a selection of different technologies were identified for evaluation as part of a proof-of-concept case study. These comprise a combined battery of broad profiling and targeted NAMs such as those described in the 2019 Computational Toxicology BluePrint [@thomas_next_2019]. Anticipated NAMs included in this first battery are a profiling NAM, high-throughput phenotypic profiling (HTPP) [@nyffeler_bioactivity_2020] which measures a large number of cellular morphological features that can be used to inform point of departure, and potentially molecular initiating events (MIEs) and targeted NAMs including high-throughput screening (HTS) assays (e.g., such as ToxCast [@judson_2010; @richard_toxcast_2016]) for *in vitro* pharmacology profiling. Collectively, these technologies have the potential to fill gaps related to molecular initiating events, including targets such as nuclear receptor targets, G-protein coupled receptors, transporters, ion channels, kinases and other enzymes, as well as oxidative and cellular stress response pathways. They can also be tailored to different routes of exposure or address specific toxicities that are more closely aligned with traditional adverse outcomes such as developmental toxicity. In addition to technologies to characterise biological activity, High Throughput Toxicokinetic (HTTK) approaches [@wetmore2015] to assess dosimetry and kinetics also form an important component of informing *in vivo* point of departure estimations based on *in vitro* bioactive concentrations.
...
Under section 5 of the Toxic Substances Control Act (TSCA), EPA's New Chemicals program helps manage potential risks to human health and the environment from chemicals new to the marketplace. For the purposes of regulation under TSCA, a chemical that is on the TSCA Inventory is considered "existing" as in a substance is known to be in US commerce. Any substance that is not on the Inventory is considered "new". For a new chemical that will be imported or manufactured in the US, the manufacturer/importer is required to file a notice to EPA, known as a [premanufacture notice (PMN)](https://www.epa.gov/reviewing-new-chemicals-under-toxic-substances-control-act-tsca/chemical-categories-used-review-new). EPA then undertakes an assessment of that chemical to determine whether it poses a risk based on its hazard and exposure profile. One of the resources EPA makes use of during that assessment are the New Chemical Categories (NCC). These are a set of categories whereby substances with shared chemical and toxicological properties have been grouped together. Many of the categories are defined based on their structural characteristics such as functional groups and physicochemical parameters. In certain cases, the categories may provide indications of what ecotoxicological or toxicological hazards might be associated with that category thereby providing recommendations of the types of data that would be informative for an assessment. There are currently 56 categories which were last updated in August 2010 and are described in narrative form in a pdf document available for download (https://www.epa.gov/reviewing-new-chemicals-under-toxic-substances-control-act-tsca/chemical-categories-used-review-new). As part of this case study, the 56 categories were first re-implemented into a machine-readable format making use of structure query syntax (as Simplified molecular-input-entry-system (SMILES) arbitrary target specification (SMARTS)) to facilitate profiling of substances into one or more of the categories. The xml file of the NCC encoded in version 4.5 of the [OECD Toolbox](qsartoolbox.org) [@schultz_2018] was exported and formed the foundation of creating an independent ruleset for the NCC which was augmented for certain categories and coupled with predictions of physicochemical parameters to provide a means of profiling the TSCA landscape into their respective NCCs.

*ClassyFire*

All substances were also assigned into primary categories using the structure-based chemical taxonomy known as ClassyFire developed by the Wishart group [@djoumbou2016]. ClassyFire assigns chemicals into a taxonomy consisting of \>4800 different categories. The taxonomy comprises 11 different levels such as Kingdom, SuperClass, Class, SubClass etc. The webserver, accessible at (http://classyfire.wishartlab.com/), was used to query each substance by its hashed International Chemical Identifier (InChIKey) [@heller_inchi_2015] and assign it into Kingdom-SubClass levels. For the 14,247 substances - 13,477 were assigned into Kingdom 'Organic compounds', 593 into Kingdom 'Inorganic' and the remaining 179 returned no information. These 179 were assigned into an arbitary class of "Other" to facilitate ongoing processing.

### Secondary and Terminal categorisation

Since structural diversity was likely to be high within a primary category – an approach was needed to balance maximising 'within category' structural similarity relative to the total number of terminal categories. Hierarchical clustering (using Ward's criterion [@ward1963]) on the basis of Morgan chemical fingerprints [@rogers_2010] (using radius 3 and bitvector length 1024) was applied to those primary categories where membership exceeded 65 substances. The first generation of clusters was taken as the secondary category. Sixty-five was an arbitrary but pragmatic threshold based on the distribution of membership size. Thirty-nine primary categories had a membership size less than 65. Primary categories whose membership was less than 65 were termed terminal categories. Primary categories containing more than 65 membership underwent further subcategorisation into secondary categories by applying hierarchical clustering. These secondary categories were hence termed terminal categories. *Note: In the figures herein, terminal categories originating from a primary category will have a designation of primary root, nan, e.g. 'Acetylides, nan' as a label as opposed to a terminal category from a secondary category which would be designated as 'Anthracenes, 2.0'.*

## Representative substance identification {#sec-maxmin}

The medoid was taken as the nominally representative substance for a given terminal category. This was defined as the substance with the minimum pairwise distance from all other members of that category. This was used as an initial seed to then identify and additional set (up to 5) of structurally diverse substances within the category on the basis of their Morgan chemical fingerprints. The approach used in this case was the MaxMin procedure [@ashton_2002] within RDKit. As described in the RDKit documentation, the algorithm works as follows:
...
The OPERA expert system [@mansouri2018] version 2.8 was used to generate a range of physicochemical property predictions for all substances in the dataset. LogKow (octanol water partition coefficient), melting point, boiling point and vapour pressure predictions were generated since these would provide a means of determining physical form at room temperature and volatility (both considerations for technical testing potential). Substances were standardised into QSAR READY format so that stereochemistry, salt forms etc. were removed from the substances under consideration. Accordingly some substances could not be converted into QSAR READY format. Merging OPERA predictions with the starting dataset found that 946 substances were not associated with any OPERA prediction either because substances were mixtures (6), were too large in size to be processed (12), could not be converted into a molecular object by RDKit (51)[^1] or were inorganic (588) or organometallic (289) in nature. Predictions were possible for up to 13,299 substances from the starting landscape which were carried forward for processing and analysis.

[^1]: These were mainly mixed metal, metal salt forms or Boron containing substances

## Creation of a constrained landscape

The landscape was constrained to only consider substances that were potentially screenable in the NAM suite. Screenability was based on 3 main considerations: 1) (physico)chemical characteristics, 2) procurability and 3) out of scope due to existing NAM data. The physicochemical characteristics captured physical state at room temperature and pressure. These were inferred based on the OPERA predictions generated (see Section on [OPERA physicochemical parameters](#sec-opera)). Melting and boiling point thresholds as outlined in the [US EPA Sustainable Futures Program](https://www.epa.gov/sites/default/files/2015-05/documents/05-iad_discretes_june2013.pdf) were used to determine whether a substance was likely to be a solid, liquid or gas at room temperature and pressure. Volatility was modelled using vapour pressure (VP). If a substance had a logVP less than 2 mmHg (VP\>100 mmHg), then it was considered screenable in aqueous screening assays. The Lipinski rule of 5 [@lipinski_2001] uses simple heuristics to denote oral absorption namely LogKow \<5, MW \<500 and Hydrogen bond donor or acceptors (\< 10 or \<5). If any of these criteria are unmet, this is referred to as a Lipinski failure. For the purposes of this study, if the number of Lipinski failures was less than 3, than a substance was considered screenable. Certain elements present challenges to stability in solvent for analytical verification. Only substances containing the following elements, C, H, N, O, P, S, Halogens and Si when not adjacent to an O, were tagged for consideration. Procurability was estimated based on the number of vendors listed in [PubChem](https://pubchem.ncbi.nlm.nih.gov/) for a given substance. The vendor count needed to be greater than 1. Out of scope referred to the substance having already been tested in the ToxCast screening programme. Considering all these aspects, the landscape was filtered to only retain substances that were tagged as screenable. For this set of substances, medoids and MaxMin substances were also identified using the same approach as already described in Section [Representative substance identification](#sec-maxmin).

## Other considerations

Aside from selection of representative substances from the full and constrained landscapes on the basis of the outlined considerations, several other aspects were also used to inform the final selection process. Analytical method detection amenability predictions for liquid-chromatography mass spectrometry (LMCS) were generated using the QSAR model developed by Lowe et al. [@lowe_2021] to provide an indication of which chemicals lent themselves to aqueous-based screening. Structural considerations based on lists of substances on the Dashboard known to be potentially explosive or highly reactive were also tagged. Category membership size was another consideration - size bins were arbitarily set at less than 20 members, between 20-70, between 70-150, between 150-300 and finally between 300-600 based on inspection of the membership count distribution. A final manual check was performed to review all proposed candidate substances for testing following application of all the aforementioned considerations. @tbl-constrained summarises the considerations defining the constrained landscape.

|                                     |                                           |                                                                                                                                                           |     |
|--------------|--------------|-------------------------------|--------------|
| Physical form                       | Liquid                                    | melting point $\leq$ 20°C                                                                                                                                 |     |
|                                     | Solid                                     | melting point \> 20°C and boiling point \>20°C                                                                                                            |     |
| Vapour pressure                     | Non-volatile to semi-volatile             | vapour pressure \< 100 mmHg                                                                                                                               |     |
| Lipinski's Rules                    | Failures \< 3                             | Lipinski's rules denote likely oral absorption based on LogKow \< 5, MW \< 500 and Hydrogen bond donor or acceptors (\< 10 or \< 5). 2 failures permitted |     |
...
| Laboratory handling                 | Exclude reactive and explosive substances | Identified by manual chemistry review and association with chemical lists                                                                                           |     |
...
### Predicted Toxicity Profile using VEGA

Predictions were generated using the VEGA QSAR Application v.1.2.3 for all human health related endpoints. Those endpoints that gave rise to binary outcomes were carried forward in the analysis to create a fingerprint representation of all 24 endpoint-model combinations for a given chemical. The endpoints considered were Ames mutagenicity, developmental toxicity, carcinogenicity, skin sensitisation, skin and eye irritation, *in vitro* and *in vivo* micronucleus activity and hepatoxicity. Some endpoints comprised 2 or more models (developmental toxicity) whereas others such as hepatoxicity comprised a single model. This fingerprint representation was evaluated from the perspective of the terminal categories 1) to visually evaluate the consistency in the toxicity profiles across a category and 2) to identify what endpoints were overrepresented in specific categories.
...
Using a deep multitask learning approach based on graph neural networks combined with attention [@Ramsundar-et-al-2019] developed in Adams et al., (2024) *under review*, hitcall predictions were made for the TSCA landscape to profile the likely NAM profile with respect to a multi-component assay platform within ToxCast. Selected assays focused on one assay platform, Attagene (ATG), which provides a multiplexed read-out of 80 transcription factors in human HepG2 hepatoma cells [@medvedev_2018].

### Availability of *in vivo* toxicity data within terminal categories

*In vivo* oral data (non-cancer and reproductive/developmental) points of departure expressed as the 25^th^ percentile of human benchmark dose (BMDh) values were taken from the supplementary files (Table S5 within the Excel files) of Aurisano et al [@aurisano_probabilistic_2023] and used to assess the availability of points of departure for the full landscape and the candidate set as well as the range of potencies. Potencies were also compared across different terminal categories.

# Data Analysis and Code Availability

Data processing was conducted using the Anaconda distribution of Python 3.9 and associated libraries, NumPy [@harris_2020], Pandas [@team_pandas-devpandas_2023], Scikit-learn [@pedregosa_scikit-learn_2011], Matplotlib [@hunter2007] and Seaborn [@waskom_mwaskomseaborn_2022]. Jupyter Notebooks [@Kluyver2016jupyter], scripts and datasets will be made available on github and on Figshare upon publication.
...
ax[0].set_xticklabels(df1['index'][:10],  rotation = 60,ha="right", fontsize = 18)
ax[1].set_xticklabels(df1['index'][10:50],  rotation = 60,ha="right", fontsize = 18)
ax[0].set_ylabel('Frequency', fontsize =22)
ax[1].set_ylabel('Frequency', fontsize =22)
ax[1].set_xlabel('NCC', fontsize =22)
...
ax1[0].set_xticklabels(cl['index'][:10],  rotation = 60,ha="right", fontsize = 18)
cl.iloc[10:,:].ClassyFire.plot.bar(ax = ax1[1])
ax1[1].set_xticklabels(cl['index'][10:],  rotation = 60,ha="right", fontsize = 18)
ax1[0].set_ylabel('Frequency', fontsize =22)
ax1[1].set_ylabel('Frequency', fontsize =22)
ax1[1].set_xlabel('ClassyFire primary categories', fontsize =22)
ax1[0].tick_params(axis='both', which='major', labelsize=18)
ax1[1].tick_params(axis='both', which='major', labelsize=18)
plt.tight_layout();
```

Inspection of the substances without NCC assignment found that these were most associated with ClassyFire Classes including 'Benzenes', 'Organonitrogen compounds', 'Fatty Acyls', 'Carboxylic acids', 'Organooxygen, organometalloid and naphthalenes'.

## Terminal categories

Medoids were initially calculated for each of the 68 primary categories. The pairwise Jaccard distances between the medoid and the remaining members of the category were also computed. These pairwise distances were calculated as a potential consideration for candidate chemical selection later; if the medoid was not a viable candidate due to previous screening or practical considerations like procurability, a close match on the basis of Jaccard distance could serve as an alternative. A primary category containing less than 65 members was effectively named as the terminal category. For the primary categories with greater than 65 members, of which there were 29, a hierarchical clustering was performed using Morgan fingerprints as inputs to create secondary categories. The first generation of clusters were cut based on visual inspection of the dendrogram. These were arbitrarily tagged by 1,2,3 etc. such that a secondary category was named using the primary category root + its cluster designation e.g. (Azobenzenes, 1.0). After clustering, 141 secondary categories were created from the starting 29 primary categories. The final set of terminal categories was made up of 39 primary categories that contained less than 65 members and 141 secondary categories, making a total of 180 terminal categories. @fig-terminal shows the 37 terminal categories that have a membership exceeding 100 substances.
...
value_counts_aggregated[(value_counts_aggregated >100) & (value_counts_aggregated < 750) ].plot(kind = 'barh', fontsize = 18)
...
The TSCA landscape was projected onto 2D using a t-SNE plot of Morgan fingerprint inputs and colour coded in 2 ways; 1) using a tag of "screenable" to permit a comparison of the structural coverage of the screenable library relative to the full landscape and 2) using a tag of the proposed candidates for testing. @fig-tsne (a) shows the screenable tag where 1 denotes substance that is member of the screenable inventory whereas @fig-tsne (b) shows the structural coverage tagged by 1 to indicate proposed candidate substance for NAM testing. @fig-tsne (a) shows how representative the constrained screenable library is of the larger landscape from a chemical structure perspective. In @fig-tsne (b) the proposed candidate (denoted as final_picks) are reasonably evenly distributed throughout the landscape suggesting that from a structural perspective, they offer promise of being broadly representative of the TSCA landscape as a whole. There was a noticeable island of 12 substances within the landscape that were determined to be not screenable in some instances due to Lipinski failures or lack of vendor. The ClassyFire classes for this set of 12 substances were 'Phenylpropanoids and polyketides', 'Carboxylic acids and derivatives','Benzene and substituted derivatives' and 'Benzofurans'. No substances were selected from northmost region of the landscape - most of the substances belonged to the ClassyFire class of prenol lipids, of the \~400 substances across the 2 terminal categories from this class, only 3 substances were selected which were structurally different (capturing NCC neutral organics compared with aldehydes, esters, thiols).
...
```{python}
#| label: fig-tsne
#| fig-cap: Selected substances within the (a) constrained Landscape or (b) whole TSCA Landscape. a) orange denotes amenability to aqueous-based *in vitro* screening per the limits described in @tbl-constrained. In b) orange denotes the proposed candidate (denoted as "final_picks").

fig, ax = plt.subplots(1,2,figsize = (10,9))
tsne_df['final_picks'] = tsne_df['final_picks'].astype(int)
sns.scatterplot(x="tsne_x", y="tsne_y",
                     data=tsne_df, hue = 'screenable', s = 10, ax = ax[0])
sns.scatterplot(x="tsne_x", y="tsne_y",
                     data=tsne_df, hue = 'final_picks', s = 10, ax = ax[1])
...
#| label: fig-opera
#| fig-cap: Physicochemical property prediction distributions between full inventory and candidate selections
...
## Predicted toxicity profile from VEGA

VEGA predictions were feasible for 12,928 substances of the TSCA landscape substances. These were summarised in a bit vector format so that each substance was associated with a fingerprint made up of all possible endpoint-models present. There were 24 endpoint-model combinations encompassing 9 different endpoints. 297 of the 318 picked substances were included in this set. They were associated with positive predictions across all of the 24 endpoint-model positive predictions. @fig-vega shows the percentage of positive predictions across the picked substances compared with the full TSCA landscape highlighting that the picked substances cover the full scan of the endpoints captured though are typically overrepresented.

```{python}
#| label: fig-vega
#| fig-cap: Percentage of positive predictions across the full and candidate set
result = pd.read_csv(interim_dir+'vega_props.csv')

(result
 .rename(columns = {'Unnamed: 1': "VEGA percentage counts", 'Unnamed: 0': 'Endpoint-Model'})
 .pivot(index = 'Endpoint-Model', columns = 'VEGA percentage counts', values = '0')
 .drop('final_picks')
 .plot(kind = 'barh', figsize = (10,9)).legend(loc = 'best', bbox_to_anchor=(1, 1))
);
```

Pairwise distance matrices of the VEGA fingerprints were then computed to explore the extent to which there was consistency in endpoint-model profile across each terminal category. Four terminal categories are illustrated in @fig-vega1 to show the extent to which profiles varied.

```{python}
import pickle

dists = pickle.load(open(interim_dir+ "dists_vega_250624.pkl", "rb" ) )
```
...
@fig-vega1 (a - d) represent terminal categories ('Pyridines and derivatives', 1.0), ('Benzene and substituted derivatives', 9.0), ('Quinolines and derivatives', 3.0') and ('Lactones', nan) respectively.

```{python}
#| label: fig-vega-clust
...
```{python}
clusters_df = pd.read_csv(interim_dir+'vega_pyridines_deriv.csv', index_col = [0])
```

For terminal category ('Pyridines and derivatives', 1.0), there are at least 3 clusters of substances (@fig-vega-clust) which shared a common alert profile as shown in @tbl-pyridines. All three clusters shared commonality in hepatotoxicity, carcinogenicity and skin sensitisation whereas in cluster 1, skin irritation featured vs. in cluster 2 eye irritation and developmental toxicity were more prominent endpoints. On the other hand, there were at least 4 clusters (@fig-vega-dend) within terminal category ('Quinolines and derivatives', 3.0) which shared common endpoint profiles as shown in @tbl-quinolines. Hepatotoxicity featured in all 4 clusters as did carcinogenicity but skin sensitisation was only an endpoint of concern in cluster 2 and Ames mutagenicity was only an endpoint of concern in cluster 1.

| cluster number | endpoint-model                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
|------------------|----------------------------------------------------|
| 1              | 'Mutagenicity (Ames test) CONSENSUS model-prediction','Developmental Toxicity model (CAESAR)-prediction', 'Carcinogenicity model (CAESAR)-prediction', 'Carcinogenicity model (ISS)-prediction', 'Carcinogenicity model (IRFMN-ISSCAN-CGX)-prediction', 'Carcinogenicity model (IRFMN-Antares)-prediction', 'Carcinogenicity oral classification model (IRFMN)-prediction', 'Skin Sensitization model (CAESAR)-prediction', 'Skin Sensitization model (IRFMN-JRC)-prediction', 'Skin Sensitization model (NCSTOX)-prediction', 'Skin Sensitization model (TOXTREE)-prediction', 'Skin Sensitization (CONCERT/Kode)-prediction', 'Skin Irritation (CONCERT/Coral)-prediction', 'Skin Irritation model (CONCERT/SarPy)-prediction', 'In vitro Micronucleus activity (IRFMN-VERMEER)-prediction', 'Hepatotoxicity model (IRFMN)-prediction' |
| 2              | 'Developmental Toxicity model (CAESAR)-prediction', 'Developmental/Reproductive Toxicity library (PG)-prediction', 'Carcinogenicity model (IRFMN-ISSCAN-CGX)-prediction', 'Carcinogenicity oral classification model (IRFMN)-prediction', 'Carcinogenicity inhalation classification model (IRFMN)-prediction', 'Skin Sensitization model (CAESAR)-prediction', 'Skin Sensitization model (IRFMN-JRC)-prediction', 'Skin Sensitization model (NCSTOX)-prediction', 'Skin Sensitization model (TOXTREE)-prediction', 'Skin Sensitization (CONCERT/Kode)-prediction', 'Eye Irritation (CONCERT/Kode)-prediction', 'In vitro Micronucleus activity (IRFMN-VERMEER)-prediction', 'Hepatotoxicity model (IRFMN)-prediction'                                                                                                                   |
| 3              | 'Developmental Toxicity model (CAESAR)-prediction', 'Carcinogenicity model (CAESAR)-prediction', 'Carcinogenicity model (IRFMN-ISSCAN-CGX)-prediction', 'Carcinogenicity model (IRFMN-Antares)-prediction', 'Carcinogenicity oral classification model (IRFMN)-prediction', 'Skin Sensitization model (CAESAR)-prediction', 'Skin Sensitization model (IRFMN-JRC)-prediction', 'Skin Sensitization model (NCSTOX)-prediction', 'Skin Sensitization model (TOXTREE)-prediction', 'Skin Sensitization (CONCERT/Kode)-prediction', 'In vitro Micronucleus activity (IRFMN-VERMEER)-prediction', 'Hepatotoxicity model (IRFMN)-prediction'                                                                                                                                                                                                   |

: VEGA profiles for terminal category ('Pyridines and derivatives', 1.0) {#tbl-pyridines}
...
```{python}
clusters_c_df = pd.read_csv(interim_dir+'vega_quinolines_derivs.csv', index_col = [0])
```

| cluster number | alerts                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
|------------------------------------|------------------------------------|
| 1              | 'Mutagenicity (Ames test) CONSENSUS model-prediction','Developmental Toxicity model (CAESAR)-prediction', 'Carcinogenicity model (IRFMN-ISSCAN-CGX)-prediction', 'Carcinogenicity oral classification model (IRFMN)-prediction', 'Carcinogenicity inhalation classification model (IRFMN)-prediction', 'Skin Sensitization model (CAESAR)-prediction', 'Skin Sensitization model (IRFMN-JRC)-prediction', 'Skin Sensitization (CONCERT/Kode)-prediction', 'Hepatotoxicity model (IRFMN)-prediction'                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 2              | 'Mutagenicity (Ames test) CONSENSUS model-prediction', 'Developmental Toxicity model (CAESAR)-prediction', 'Developmental/Reproductive Toxicity library (PG)-prediction', 'Carcinogenicity model (CAESAR)-prediction', 'Carcinogenicity model (ISS)-prediction', 'Carcinogenicity model (IRFMN-ISSCAN-CGX)-prediction', 'Carcinogenicity oral classification model (IRFMN)-prediction', 'Carcinogenicity inhalation classification model (IRFMN)-prediction', 'Skin Sensitization model (CAESAR)-prediction', 'Skin Sensitization model (IRFMN-JRC)-prediction', 'Skin Sensitization model (NCSTOX)-prediction', 'Skin Sensitization (CONCERT/Kode)-prediction', 'Skin Sensitization (CONCERT/SarPy)-prediction', 'Eye Irritation (CONCERT/Kode)-prediction', 'In vitro Micronucleus activity (IRFMN-VERMEER)-prediction', 'In vivo Micronucleus activity (IRFMN)-prediction', 'Hepatotoxicity model (IRFMN)-prediction' |
| 3              | 'Mutagenicity (Ames test) CONSENSUS model-prediction', 'Developmental Toxicity model (CAESAR)-prediction', 'Carcinogenicity model (CAESAR)-prediction', 'Carcinogenicity model (ISS)-prediction', 'Carcinogenicity model (IRFMN-ISSCAN-CGX)-prediction', 'Carcinogenicity model (IRFMN-Antares)-prediction', 'Carcinogenicity oral classification model (IRFMN)-prediction', 'Carcinogenicity inhalation classification model (IRFMN)-prediction', 'Skin Sensitization model (CAESAR)-prediction', 'Skin Sensitization model (IRFMN-JRC)-prediction', 'Skin Sensitization model (NCSTOX)-prediction', 'Skin Sensitization model (TOXTREE)-prediction', 'Eye Irritation (CONCERT/Kode)-prediction', 'Eye Irritation (CONCERT/SarPy)-prediction', 'In vitro Micronucleus activity (IRFMN-VERMEER)-prediction', 'Hepatotoxicity model (IRFMN)-prediction'                                                                   |
| 4              | 'Mutagenicity (Ames test) CONSENSUS model-prediction', 'Developmental Toxicity model (CAESAR)-prediction', 'Carcinogenicity model (CAESAR)-prediction', 'Carcinogenicity model (ISS)-prediction', 'Carcinogenicity model (IRFMN-ISSCAN-CGX)-prediction', 'Carcinogenicity oral classification model (IRFMN)-prediction', 'Carcinogenicity inhalation classification model (IRFMN)-prediction', 'Skin Sensitization model (CAESAR)-prediction', 'Skin Sensitization model (NCSTOX)-prediction', 'Skin Sensitization (CONCERT/Kode)-prediction', 'Skin Irritation model (CONCERT/SarPy)-prediction', 'Eye Irritation (CONCERT/Kode)-prediction', 'Eye Irritation (CONCERT/KNN)-prediction', 'In vitro Micronucleus activity (IRFMN-VERMEER)-prediction', 'In vivo Micronucleus activity (IRFMN)-prediction', 'Hepatotoxicity model (IRFMN)-prediction'                                                                     |

: VEGA profiles for terminal category ('Quinolines and derivatives', 3.0) {#tbl-quinolines}
...
with open('img_cluster_1.png','wb+') as outf:
    outf.write(png1)
with open('img_cluster_2.png','wb+') as outf:
   outf.write(png2)
   

```

The alert profiles may assist with refining understanding of predicted toxicity within these structure-based categories, which are constructed pragmatically but may group substances that have small but important structural differences that lead to different predictions. For instance, in (‘Pyridines and derivatives’, 1.0), there is a difference in prediction between substances that contain a CCl~3~ group versus other chemicals in the terminal category which are still highly chlorinated but lack this particular group (@fig-pyridine-clust). Further, the alert profiles within the terminal categories may provide useful insights on the types of targets or toxicities to evaluate in the *in vitro* NAM assays.

No structural alert cluster profiles were produced for 16 terminal categories - namely ('Allenes'), ('Anthracenes', 2.0), ('Anthracenes', 3.0), ('Anthracenes', 5.0), ('Azobenzenes', 1.0), ('Azobenzenes', 2.0), ('Azobenzenes', 5.0), ('Azobenzenes', 6.0), ('Azobenzenes', 7.0), ('Azoles', 3.0), ('Benzenoids', 5), ('Lignans, neolignans and related compounds', nan), ('Organic oxygen compounds'), ('Organoheterocyclic compounds', 2.0), ('Quinolines and derivatives', 1.0) and ('Quinolines and derivatives', 2.0).

Of these, ('Allenes'), ('Azoles', 3.0) and ('Lignans, neolignans and related compounds') all had low memberships (2,4,6 respectively) which might limit the potential for a prediction to be triggered. For the other 13 terminal categories, each was associated with a number of positive endpoint-model predictions as shown in @tbl-terms. For example, terminal category ('Anthracenes', 5) was associated with 10 endpoint-models that were positive for all 16 members. The endpoints associated with these 10 model predictions were carcinogenicity, Ames mutagenicity, developmental toxicity, skin sensitisation, eye irritation and *in vitro* micronucleus activity.

|           Terminal category           | Number of positive predictions | Membership size |
|:---------------------------:|:-----------------------:|:----------------:|
...
|          ('Benzenoids', 5.0)          |               13               |       13        |
|     ('Organic oxygen compounds')      |               12               |        6        |
| ('Organoheterocyclic compounds', 2.0) |               17               |       12        |
|  ('Quinolines and derivatives', 1.0)  |               15               |        6        |
|  ('Quinolines and derivatives', 2.0)  |               17               |       11        |

: Terminal categories that could not be clustered but were associated with many positive predictions {#tbl-terms}

For the remaining terminal categories, the maximum number of positive predictions was 111 for terminal category ('Benzene and substituted derivatives', 2.0) whereas the median number of positive predictions was 9 (see @fig-vega3).

```{python}
#| label: fig-vega3
#| fig-cap: Distribution of VEGA positive prediction counts across terminal categories which were clustered

vega_counts = pd.read_csv(interim_dir+'vegacount_terminalcategories.csv', index_col = [0])
vega_counts.plot(kind = 'hist')
plt.xlabel('number of positive predictions');
```

A count of the positive predictions across the 12 endpoints was summarised across the categories to gain a perspective of the relationship between categories and the endpoints that were triggered based on the alerts. @fig-alert-tox-hm shows a clustermap of the alert endpoints as a function of the terminal categories. For example, terminal categories containing benzene compounds were typically associated with carcinogenicity, *in vivo* micronucleus and hepatotoxicity endpoints whereas organohalogen compounds or quinoline and derivatives were associated with skin sensitisation.

```{python}
import pandas as pd
import seaborn as sns
new_df= pd.read_csv(interim_dir+'vega_map.csv', index_col = [0])
...
#| fig-cap: Distribution of VEGA prediction counts across terminal categories which were clustered
...
|------------------------|:--------------:|:--------------:|:--------------:|
| ('Steroids and steroid derivatives')              |            1.0             |          31.0          | 31.0  |
| ('Phenylpropanoids and polyketides', 4.0)         |            6.0             |          96.0          | 16.0  |
| ('Organic salts')                                 |            1.0             |          16.0          | 16.0  |
...
```{python}
#| label: fig-count
#| fig-cap: 'Comparison of overall TSCA landscape and final selections in terms of the number of positive Ames or Developmental toxicity profile predictions'
...
::: {#fig-pyridine-clust layout-ncol="2"}
![Cluster 1](img_cluster_1.png){#fig-pc1 width="50%"}

![Cluster 2](img_cluster_2.png){#fig-pc2 width="50%"}

Members of the ('Pyridines and derivatives', 1.0') terminal category that fall into cluster 1 vs. cluster 2 of the structural alert profile.
:::
...
```{python}
#| label: fig-ames
#| fig-cap: t-SNE plots of TEST mutagenicity predictions for categories with members exceeding 40. All terminal categories with fewer than 40 members were aggregated into one 'miscellaneous' category.
...
```{python, fig.width="50%"}
#| label: fig-ames2
#| fig-cap: 't-SNE plots of selected categories that are overrepresented by a) Ames positive predictions or b) developmental toxicity predictions'
...
![Network graph representation of primary categories and their predicted VEGA outcomes. Network graphs of primary categories and their predicted VEGA endpoint. The linkages show the associations between different toxicity endpoints (shown by the blue circles) and the terminal categories (shown by the green circles). For example, Epoxides are associated with several endpoints including skin irritation, mutagenicity, and skin sensitisation. From these representations, such insights as to which categories are typically associated with a specific outcome can be more readily identified which are informative for prioritisation efforts.](vega_nx.png){#fig-nx}
